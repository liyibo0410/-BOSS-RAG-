...
class NodeImportMilvus(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 运行一次 或者无数次 结果都一样  file_title 删了 重写
    ...
    def _step_2_prepare_collection(self, vector_dimension: int):
        ...
        if not milvus_client.has_collection(collections_name):# 集合不存在 就创建
    ...
    def _create_chunks_collection(self, collections_name, milvus_client, vector_dimension):
        ...
        schema.add_field(field_name="parent_title", datatype=DataType.VARCHAR, max_length=100, default_value="")  # 父标题
    ...
    def _clear_chunks_by_file_title(self, client, file_title):
        ...
        try:
            file_title = escape_milvus_string(file_title) # 文件名
            client.delete(# 删除
                collection_name=milvus_config.chunks_collection,
                filter=f"file_title=='{file_title}'") # 删除 kb_chunks file_title 所对应的内容
